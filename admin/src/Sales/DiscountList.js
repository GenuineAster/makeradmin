...
class DiscountList extends CollectionNavigation {
    constructor(props) {
        super(props);
        const {search, page} = this.state;
        this.state.categories = null;

        this.collection = new Collection({type: Discount, search, page});
    ...
    render() {
        ...
        return (
            <div className="uk-margin-top">
                <h2>Produkter</h2>
                <p className="uk-float-left">Here you will find a list of all active discounts.</p>
                <Link className="uk-button uk-button-primary uk-margin-bottom uk-float-right" to="/sales/discount/add"><i className="uk-icon-plus-circle"/> Create a new discount</Link>
                ...
                <CollectionTable
                    ...
                    rowComponent={({item, deleteItem}) =>
                        <tr>
                            <td><Link to={"/sales/discount/" + item.id}>{item.name}</Link></td>
